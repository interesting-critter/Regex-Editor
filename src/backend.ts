  ...
  try {
    switch (payload.type) {
      ...
      case 'list_characters': {
        const result = await spindle.characters.list({ limit: 200, offset: 0 })
      ...
      case 'save_character': {
        await spindle.characters.update(payload.characterId, payload.patch)
        spindle.toast.success(`Character "${payload.name || 'card'}" updated successfully!`)
      ...
      case 'list_world_books': {
        const result = await spindle.world_books.list({ limit: 200, offset: 0 })
        spindle.sendToFrontend({
      ...
      case 'get_world_book': {
        const entriesResult = await spindle.world_books.entries.list(payload.worldBookId, { limit: 200, offset: 0 })
        spindle.sendToFrontend({
      ...
      case 'save_world_book_entries': {
        ...
        for (const update of updates) {
          await spindle.world_books.entries.update(update.id, { content: update.content })
        }
